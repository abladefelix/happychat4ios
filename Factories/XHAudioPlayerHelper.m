...
- (void)playAudioWithFileName:(NSString*)fileName {
    if (fileName.length > 0) {
        ...
        if (_playingFileName && [fileName isEqualToString:_playingFileName]) {//上次播放的录音
            ...
            //判断是否文件是否存在
            NSString *path = [Tool returnRecordFilePath:fileName];
            if ([[NSFileManager defaultManager] fileExistsAtPath:path]) {
                NSError *error;
                NSData *data = [NSData dataWithContentsOfFile:path];
                AVAudioPlayer *pl = [[AVAudioPlayer alloc] initWithData:DecodeAMRToWAVE(data) error:&error];
                if (error) {
                    debugLog(@"%@",error);
                }
                else {
                ...
                NSURL *URL = [NSURL URLWithString:[NSString stringWithFormat:@"%@", fileName]];
                ...
                [manager downloadTaskWithRequest:request
                                     ...
                                     destination:^NSURL *(NSURL *targetPath, NSURLResponse *response) {
                                         return [NSURL fileURLWithPath:[Tool returnRecordFilePath:fileName]];
                                     }
                               completionHandler:^(NSURLResponse *response, NSURL *filePath, NSError *error) {
                                   NSData *data = [NSData dataWithContentsOfURL:filePath];
                                   AVAudioPlayer *pl = [[AVAudioPlayer alloc] initWithData:DecodeAMRToWAVE(data) error:nil];
                                   pl.delegate = self;
                                   [pl play];
                                   self.player = pl;
                                   [[UIDevice currentDevice] setProximityMonitoringEnabled:YES];
...
- (void)sensorStateChange:(NSNotificationCenter *)notification {
    ...
    if ([[UIDevice currentDevice] proximityState] == YES) {
        //黑屏
        debugLog(@"Device is close to user");
        [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayAndRecord error:nil];
        
    } else {
        //没黑屏幕
        debugLog(@"Device is not close to user");
        [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayback error:nil];
        if (!_player || !_player.isPlaying) {
