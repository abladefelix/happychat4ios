...
- (void)_init
{
    ...
    if (!self.scrollView) {
        ...
        self.scrollView.width = screenframe.size.width;
...
- (void)reloadData
{
    ...
    CGFloat width = screenframe.size.width - self.edgeInsets.left - self.edgeInsets.right;
    CGFloat height = self.height - self.edgeInsets.top - self.edgeInsets.bottom;
    debugLog(@"%f", height);
...
//    self.pageControl.currentPage = nextPage;
    int page = ceil((self.scrollView.contentOffset.x - self.scrollView.frame.size.width/(self.count))/self.scrollView.frame.size.width);
...
//    }
    ...
    int page = ceil((scrollView.contentOffset.x - pagewidth/(self.count))/pagewidth);
//    currentPageForScrollView = page;
//    self.pageControl.currentPage = page;
    if (page==0) {
        [self performSelector:@selector(scrollToLast) withObject:nil afterDelay:0.1];
        self.pageControl.currentPage = self.count-2;
    }
    else if (page==(self.count-1)) {
        [self performSelector:@selector(scrollToFirst) withObject:nil afterDelay:0.1];
        self.pageControl.currentPage = 0;
    }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    CGFloat pagewidth = scrollView.frame.size.width;
    int currentPage = ceil((scrollView.contentOffset.x - pagewidth/ (self.count)) / pagewidth);
    if (currentPage==0) {
        [self performSelector:@selector(scrollToLast) withObject:nil afterDelay:0.1];
        self.pageControl.currentPage = self.count-2;
    }
...
-(void)scrollToLast
{
    [self.scrollView scrollRectToVisible:CGRectMake(self.scrollView.frame.size.width * (self.count-2), 0 ,self.scrollView.frame.size.width ,self.scrollView.frame.size.height) animated:NO];
}
